# Exam Server
## Описание
Данный проект является сервером для мобильного приложения "Чат". Сервер является Restful HTTP server. Взаимодействие с сервером происходит посредством отправки и получения GET и POST запросов. Данные, в свою очередь, передаются в виде JSON.

## Описание методов
* /all-messages: Получить все сообщения, имеющиеся на сервере.
  * Запрос:
    * Метод: GET.
    * login: Логин **зарегистрированного** и **авторизированного** пользователя.
    * Пример: http://localhost:50000/all-messages?login=user_login
  * Ответ:
    * messages: список сообщений. Структура каждого сообщения:
      * owner: Владелец сообщения.
      * text: Текст сообщения.
      * time: Дата и время **получения** сообщения в формате "ГГГ-ММ-ДД чч:мм:сс". 
    * Пример: {"messages": [{"owner":"user_login", "text": "Some text", "time":"2025-01-01 01:02:03"}]}
* /all-messages: Получить сообщения после указанной даты и времени.
  * Запрос:
    * Метод: POST
    * login: Логин **зарегистрированного** и **авторизированного** пользователя.
    * time: Дата и время в формате "ГГГ-ММ-ДД чч:мм:сс". 
    * Пример: {"login": "user_login", "time": "2025-01-01 01:02:03"}
  * Ответ:
    * messages: список сообщений. Структура каждого сообщения:
      * owner: Владелец сообщения.
      * text: Текст сообщения.
      * time: Дата и время **получения** сообщения в формате "ГГГ-ММ-ДД чч:мм:сс". 
    * Пример: {"messages": [{"owner":"user_login", "text": "Some text", "time":"2025-01-01 01:02:03"}]}
* /sign-in: Авторизироваться на сервере.
  * Запрос:
    * Метод: POST
    * name: Имя пользователя.
    * login: Логин **зарегистрированного** пользователя.
    * password: Пароль.
    * Пример: {"name": "user_name","login": "user_login", "password": "user_password"}
  * Ответ:
    * message: Информационное сообщение о результате операции.
    * Пример: {"message": "You are successfully logged in"}
* /sign-up:
  * Запрос:
    * Метод: POST
    * name: Имя пользователя.
    * login: Логин пользователя.
    * password: Пароль.
    * Пример: {"name": "user_name","login": "user_login", "password": "user_password"}
  * Ответ:
    * message: Информационное сообщение о результате операции.
    * Пример: {"message": "You are successfully logged in"}
* /send-message: Отправить сообщение на сервер.
  * Запрос:
    * Метод: POST
    * owner: Логин **зарегистрированного** и **авторизированного** пользователя.
    * text:
    * Пример: {"owner": "user_login","text": "Some message"}
  * Ответ:
    * message: Информационное сообщение о результате операции.
    * Пример: {"message": "You are successfully logged in"}
